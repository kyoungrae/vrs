        ...
        if($start != "none" && $end != "none"){
            Excel::create("합계 ТХ -н статистик мэдээ", function($excel) use($start, $end) {
                ...
                $excel->sheet("보고서", function($sheet) use($start, $end) {
                    ...
                    FROM VRS.REG_VEHICLE_VIEW VEN JOIN VRS.REG_OWNER_VIEW OW ON VEN.OWNER_ID=OW.ID JOIN VRS.ADDRESS_PROVINCE PO ON OW.PROVINCE_ID=PO.ID WHERE
