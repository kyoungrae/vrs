   ...
   if(!session()->has("auth")){
    ...
    public function plateSaveVehicleSearch(Request $request)
    {
       ...
       $seriesNumberCheck = DB::table("series_number")->where("NAME", $plateNo)->first();
        $seriesCheck = DB::select(DB::raw("SELECT * FROM series where (( substr(name,1,2) like 'УC%' or substr(name,1,2) like 'УJ%' or substr(name,1,2) like 'УG%')) and type='1' and id='".$seriesNumberCheck->series_id."' "));
                   //    return $seriesCheck;
