    ...
    public function createOwner(Request $request){
        ...
        try{
           ...
           // $specialnote = $request->get("specialnote");
            ...
            // }
           // $order_qty = null || $order_qty ="" ? $order_qty = 3 : $order_qty = 3;
            $address_details = "";
            if($province != ""){
        ...
        try{
            if($type == "company"){
               ...
               // $owner = Owner::where("REGISTER_NO", $register)->get();
                $owner = DB::table("REG_OWNER_VIEW")->where("REGISTER_NO", $register)->get();
                // $owner = DB::table("REG_OWNER_VIEW")
                // ->where("REGISTER_NO", 'like', '%' . $register . '%')
                // ->get();
           ...
           //     return "fghfghfghfhfgh";
              //  $owners = Owner::where("REGISTER_NO", "LIKE", $register."%")->where("TYPE_ID", "!=", 1)->get();
              //  $owners = DB::table("REG_OWNER_VIEW")->where("REGISTER_NO", $register)->get();
                $owners = DB::table("REG_OWNER_VIEW")
                ->where("REGISTER_NO", 'like', '%' . $register . '%')->where("OWNER_TYPE_ID", "!=", 1)
                ->get();
              //  return  $owners;
                if($owners->count() > 0){
                    return "true";
        ...
        if($departments->count() > 0){
         ...
         //       $serieses = Series::where("NAME", "LIKE", "ДК%")->orderBy("NAME")->get();
           // } else {
            ...
            $province = AddressProvince::where("old_id", $departments->province_id)->first();
      
            //return $departments->province_id;
                $serieses = Series::where("PROVINCE_ID", $province->id)->orderBy("NAME")->get();
               // return $serieses;
          //  }
//return $serieses;
        ...
        if($vid != ""){
            ...
            foreach ($histories as $history){
                ...
                $html .= '<tr>' .
                   ...
                   '<td><a href="'.url('/archive/document/'.$history->archive_no).'" target="_blank" onclick="window.open(&quot;'.url('/archive/documentAr/'.$history->archive_no).'/&quot);">'.$history->archive_no.'</a></td>'.
