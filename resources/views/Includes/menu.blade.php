...
@if(\App\Http\Controllers\BaseController::checkChangePassword($authId) < 30)
    ...
    <nav class="nav">
        ...
        @if(\App\Http\Controllers\BaseController::hasMenuShow("/mehanizm/search", 1, \App\Http\Controllers\BaseController::enc($authPositionId)))
            <a title="기타 차량 번호 검색" href="{{ url('/mehanizm/search') }}" onclick="location.href='{{ url('/mehanizm/search') }}';" style="color: rgba(255, 255, 255, 0.5) !important;margin-top: 10px;border-radius: 3px; transition: all 0.2s ease-in-out; position: relative; padding: 0;width: 40px; height: 40px; display: flex; align-items: center; justify-content: center; line-height: 1; font-size: 30px;" class="<?php $url = 'http://' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI']; $activeMenu=""; if (strpos($url,"search") != false) { echo $activeMenu="active"; } ?>"><i class="la la-search"></i></a>
        @endif
        ...
        @if(\App\Http\Controllers\BaseController::hasMenuShow("/reference/orderednumbers", 1, \App\Http\Controllers\BaseController::enc($authPositionId)))
            <a title="예약 번호 검색" href="{{ url('/reference/orderednumbers') }}" onclick="location.href='{{ url('/reference/orderednumbers') }}';" style="color: rgba(255, 255, 255, 0.5) !important;margin-top: 10px;border-radius: 3px; transition: all 0.2s ease-in-out; position: relative; padding: 0;width: 40px; height: 40px; display: flex; align-items: center; justify-content: center; line-height: 1; font-size: 30px;" class="<?php $url = 'http://' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI']; $activeMenu=""; if (strpos($url,"orderednumbers") != false) { echo $activeMenu="active"; } ?>"><i class="la la-filter"></i></a>
        @endif

        @if(\App\Http\Controllers\BaseController::hasMenuShow("/plate/print", 1, \App\Http\Controllers\BaseController::enc($authPositionId)))
            <a title="번호판 인쇄" href="{{ url('/plate/print') }}" onclick="location.href='{{ url('/plate/print') }}';" style="color: rgba(255, 255, 255, 0.5) !important;margin-top: 10px;border-radius: 3px; transition: all 0.2s ease-in-out; position: relative; padding: 0;width: 40px; height: 40px; display: flex; align-items: center; justify-content: center; line-height: 1; font-size: 30px;" class="<?php $url = 'http://' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI']; $activeMenu=""; if (strpos($url,"print") != false) { echo $activeMenu="active"; } ?>"><i class="la la-print"></i></a>
        @endif
    </nav>
@endif