                                            ...
                                            <div class="row row-xs align-items-center mg-b-5">
                                                <div class="col-lg-12 col-md-12 col-sm-12">
                                                    <select class="form-control select2 required-input" name="operation" required>
                                                        <option value="1" {{ isset($op) ? $op == "1" ? "selected" : "" : "" }}>qPay & socialPay</option>
                                                        <option value="2" {{ isset($op) ? $op == "2" ? "selected" : "" : "" }}>키오스크</option>
                                                        <option value="3" {{ isset($op) ? $op == "3" ? "selected" : "" : "" }}>은행</option>
                                                        <option value="4" {{ isset($op) ? $op == "4" ? "selected" : "" : "" }}>은행 POS</option>
                                                        <option value="5" {{ isset($op) ? $op == "5" ? "selected" : "" : "" }}>전자 요청으로</option>
                                                    </select>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-3">
                                            <div class="row row-xs align-items-center mg-b-5">
                                                <div class="col-lg-4 col-md-12 col-sm-12">
                                                    <label class="form-label mg-b-0 required-input"> 담당자</label>
                                                </div>
...
<script>
    ...
    function exportToExcel() {
        ...
        $.ajax({
            type: "GET",
            url: vrsUrl("/api/report/epay/users/@if(ISSET($op)){{ $op }}@else{{ "none" }}@endif/@if(ISSET($userId)){{ $userId }}@else{{ "none" }}@endif/@if(ISSET($startDate)){{ $startDate }}@else{{ "none" }}@endif/@if(ISSET($endDate)){{ $endDate }}@else{{ "none" }}@endif"),
            success: function (data) {
               // console.log(data);
                @if(ISSET($startDate))
                    window.location = vrsUrl("/api/report/epay/users/@if(ISSET($op)){{ $op }}@else{{ "none" }}@endif/@if(ISSET($userId)){{ $userId }}@else{{ "none" }}@endif/@if(ISSET($startDate)){{ $startDate }}@else{{ "none" }}@endif/@if(ISSET($endDate)){{ $endDate }}@else{{ "none" }}@endif");
