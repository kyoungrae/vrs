                                        ...
                                        <div class="col-2">
                                            <div class="row row-xs align-items-center mg-b-5">
                                                ...
                                                <div class="col-lg-8 col-md-12 col-sm-12">
                                                    <input id="start" name="start" required type="text" value="{{ isset($startDate) ? $startDate : "" }}" class="form-control" autocomplete="off">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-2">
                                            <div class="row row-xs align-items-center mg-b-5">
                                                ...
                                                <div class="col-lg-8 col-md-12 col-sm-12">
                                                    <input id="end" name="end" required type="text" value="{{ isset($endDate) ? $endDate : "" }}" class="form-control" autocomplete="off">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-3">
                                    ...
                                    @if(ISSET($results))
                                        @foreach($results as $result)
                                            <tr>
                                                ...
                                                <td>{{ \Carbon\Carbon::parse($result->created_date)->format("Y-m-d H:i:s") }}</td>
...
<script>
    ...
    function exportToExcel() {
        ...
        $.ajax({
            type: "GET",
            url: vrsUrl("/api/report/certificate/@if(ISSET($curr_user)){{ $curr_user }}@else{{ "none" }}@endif/@if(ISSET($startDate)){{ $startDate }}@else{{ "none" }}@endif/@if(ISSET($endDate)){{ $endDate }}@else{{ "none" }}@endif"),
            success: function (data) {
                @if(ISSET($startDate))
                    $(".avtoteeverPreloader").fadeOut();
                $(".containerBody").fadeIn();
                window.location = vrsUrl("/api/report/certificate/@if(ISSET($curr_user)){{ $curr_user }}@else{{ "none" }}@endif/@if(ISSET($startDate)){{ $startDate }}@else{{ "none" }}@endif/@if(ISSET($endDate)){{ $endDate }}@else{{ "none" }}@endif");
                @else
                    $(".avtoteeverPreloader").fadeOut();
                $(".containerBody").fadeIn();
