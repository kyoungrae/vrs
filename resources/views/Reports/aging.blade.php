                                                    ...
                                                    <select class="form-control select2" name="province" id="province">
                                                        ...
                                                        @if(ISSET($provinces))
                                                            @foreach($provinces as $province)
                                                                <option value="{{ $province->id }}" {{ isset($curr_pro) ? $curr_pro == $province->id ? "selected" : "" : "" }}>{{ $province->name }}</option>
                                                            @endforeach
                                                        @endif
                                                    </select>
                                                </div>
                                ...
                                <table id="referenceTable" class="display responsive nowrap" style="width:100%;">
                                    ...
                                    @if(ISSET($results))
                                        @foreach($results as $result)
                                            <?PHP
                                                $total = $result->nas_0_3 + $result->nas_4_6 + $result->nas_7_10 + $result->nas_10_ikh;
                                            ?>
                                            <tr>
                                                <td>{{ $loop->iteration }}</td>
                                                <td>{{ $result->name }}</td>
                                                <td>{{ $result->purpose_name }}</td>
                                                <td>{{ $result->nas_0_3 }}</td>
                                                <td>{{ round(100 * $result->nas_0_3 / $total, 1) }}%</td>
                                                <td>{{ $result->nas_4_6 }}</td>
                                                <td>{{ round(100 * $result->nas_4_6/ $total, 1) }}%</td>
                                                <td>{{ $result->nas_7_10 }}</td>
                                                <td>{{ round(100 * $result->nas_7_10/ $total, 1) }}%</td>
                                                <td>{{ $result->nas_10_ikh }}</td>
                                                <td>{{ round(100 * $result->nas_10_ikh / $total, 1) }}%</td>
                                                <td>{{ $total }}</td>
                                            </tr>
                                        @endforeach
                                    @endif
...
<script>
    ...
    $( document ).ready(function() {
        @if(ISSET($startDate))
            $("#start").val('{{ $startDate }}');
        @else
            $("#start").val('{{ \Carbon\Carbon::now()->addDay(-7)->format("Y-m-d") }}');
        @endif
