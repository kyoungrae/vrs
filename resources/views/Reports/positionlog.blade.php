                                                    ...
                                                    <select class="form-control select2" name="position">
                                                        @if(ISSET($positions))
                                                            @foreach($positions as $position)
                                                                <option value="{{ $position->id }}" {{ isset($pos) ? $pos == $position->id ? "selected" : "" : "" }}>{{ $position->name }}</option>
                                                            @endforeach
                                                        @endif
                                                    </select>
                                                </div>
                                    ...
                                    @if(ISSET($logs))
                                        @foreach($logs as $log)
                                            <tr>
                                                ...
                                                <td>{{ \Carbon\Carbon::parse($log->createddate)->format("Y-m-d H:i:s") }}</td>
                                                <td>{{ $log->old_position_name }}</td>
                                                <td>{{ $log->position_name }}</td>
                                                <td>{{ $log->op_name }}</td>
                                                <td>{{ $log->firstname ." ". $log->lastname }}</td>
                                                <td>{{ $log->u_firstname ." ". $log->u_lastname }}</td>
                                            </tr>
                                        @endforeach
                                    @endif
                                    </tbody>
