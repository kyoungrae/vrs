                ...
                <div class="row row-sm">
                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12">
                        <h6 class="card-title">
                            <div class="row">
                                <div class="col-lg-12 col-md-12 col-sm-12">
                                    {{ \App\Helpers\TranslationHelper::translate("САЛБАРЫН ТАЙЛАН") }}
                                </div>
                            </div>
                        </h6>
                        <div class="row row-sm">
                                    ...
                                    @if(ISSET($results))
                                        @foreach($results as $result)
                                            <tr>
                                                ...
                                                <td>{{ \App\Archive::getDepartmentName($result->archive_abbr) }}</td>
                                                <td>{{ \Carbon\Carbon::parse($result->archive_date)->format("Y-m-d") }}</td>
                                                <td>{{ $result->new_v }}</td>
                                                <td>{{ $result->move_v }}</td>
                                                <td>{{ $result->move_plate_v }}</td>
                                                <td>{{ $result->cert_change_v }}</td>
                                                <td>{{ $result->cert_again_v }}</td>
                                                <td>{{ $result->limit_v }}</td>
                                                <td>{{ $result->remove_v }}</td>
                                                <td>{{ $result->change_plate_v }}</td>
                                                <td>{{ $result->change_plate_two_v }}</td>
...
<script>
    ...
    function exportToExcel() {
        ...
        $.ajax({
            type: "GET",
            url: vrsUrl("/api/report/department/@if(ISSET($curr_dep)){{ $curr_dep }}@else{{ "none" }}@endif/@if(ISSET($startDate)){{ $startDate }}@else{{ "none" }}@endif/@if(ISSET($endDate)){{ $endDate }}@else{{ "none" }}@endif"),
            success: function (data) {
                @if(ISSET($startDate))
                    $(".avtoteeverPreloader").fadeOut();
                $(".containerBody").fadeIn();
                window.location = vrsUrl("/api/report/department/@if(ISSET($curr_dep)){{ $curr_dep }}@else{{ "none" }}@endif/@if(ISSET($startDate)){{ $startDate }}@else{{ "none" }}@endif/@if(ISSET($endDate)){{ $endDate }}@else{{ "none" }}@endif");
                @else
                    $(".avtoteeverPreloader").fadeOut();
                $(".containerBody").fadeIn();
